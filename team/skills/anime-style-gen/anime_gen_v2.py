...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 anime_gen_v2.py <image_path> <style_prompt> <output_path>")
        sys.exit(1)
    ...
    # The DEFINITIVE "Trading Card First" Instruction Set
    rules = (
        "TASK: Transform the original photo into a stylized 'Trading Card'. "
        "1. DEFAULT CARD FORMAT: The entire output must be framed within a decorative, high-quality trading card border (Pokemon style). "
        "2. STYLE: Apply the specified style (e.g., Studio Ghibli) to the artwork INSIDE the card. "
    ...
    try:
        ...
        if match:
            ...
            with open(output_path, "wb") as f:
                f.write(img_data)
            print(f"✅ Card Image saved: {output_path}")
